            ...
            for i_continuous_states, n_continuous_states in enumerate(ns_states):
                for i_discrete_states, n_discrete_states in enumerate(ns_discrete_states):
                    for i_iters, n_iters in enumerate(ns_iters):
                        ...
                        res_save_path_new = model_save_prefix + '_causal.pkl'

                        if (not overwrite_results) and os.path.isfile(res_save_path_new):
                        ...
                        assert os.path.isfile(res_save_path), 'Same speed model not found: ' + res_save_path
                        ...
                        neural_SLDS.causal_transform(
                            emissions=X_test, 
                            inputs=X_input_test)

                        with open(res_save_path_new, 'wb') as f:
                            pickle.dump({
                                'causal_continuous_states' : neural_SLDS.causal_continuous_states,
                                'causal_discrete_states'   : neural_SLDS.causal_discrete_states,
